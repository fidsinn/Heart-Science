...
The following classification tree shows that the data was first split at the result of [thal](#thal). Furthermore split at [oldpeak](#oldpeak), [chol](#chol), [exang](#exang), [thalach](#thalach) and [ca](#ca).  
...
* 6% of all patients were predicted to have a normal thal, exercise induced angina and thalach >= 140
...
```{r Model.random.forest, echo=FALSE}
...
```{r Model.random.forest2, echo=FALSE}
Conf.random.forest$table %>%
  knitr::kable() %>% 
  kableExtra::kable_styling(full_width = FALSE)
Sens.random.forest <- confusionMatrix(Model.random.forest, testing$disease)$byClass[c("Sensitivity")]
